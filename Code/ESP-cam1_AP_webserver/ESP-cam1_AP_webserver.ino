...
void handleRoot() {
  ...
  s.replace("{DHT_DATA}", readDHTData()); // Replace {DHT_DATA} with DHT sensor data
  server.send(200, "text/html", s); // Send the modified web page
}
...
void setup()
{
  ...
  dht.begin();
  pinMode(LEDFLASH, OUTPUT);  // Set GPIO4 as an output
  ...
  WiFi.begin("PTCL FLASH FIBER", "12341212");
